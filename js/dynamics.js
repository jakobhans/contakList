...
function createStorageObject(){
	...
	for(var n=0;n<localStorage.length;n++){
		if(!localStorage.getItem(i)){
			...
			var pseudoVar=localStorage.getItem(i);
...
function getDataElements(){
	var idKey=localStorage.length;
	while(localStorage.getItem(idKey)){
		idKey++;
	}
...
function submitContact(){
	...
	if(contact_name===""||(contact_name===""&&contact_email==="")||(contact_name===""&&contact_phone==="")||(contact_email===""&&contact_phone==="")){
		...
		localStorage.setItem(contact_id,data);
		return true;
	}
}
...
function deleteContact(contact_id){
	...
	for(var i=0;i<contacts.length;i++){
		if(contacts[i].getStorage()===contact_id){
			...
			if(confirm_box===true){
				localStorage.removeItem(contacts[i].getStorage());
