...
function createStorageObject(){for(var b=[],a=0,c=0;c<localStorage.length;c++){if(localStorage.getItem(a)){var d=localStorage.getItem(a).split("|&|");b[c]=new ContactElement(d[0],c,d[1],d[2],a)}else c--;a++}return b}
...
function displayContacts(){createStorageObject();for(var b="",a=0;a<localStorage.length;a++)b+=displayContactHTML(a);document.getElementById("contacts").innerHTML=b}function getDataElements(){for(var b=localStorage.length;localStorage.getItem(b);)b++;document.getElementById("id_field").value=b;if(localStorage.length>0){document.getElementById("search_contacts").disabled=false;displayContacts()}else document.getElementById("search_contacts").disabled=true;return true}
...
function deleteContact(b){var a=[];b=parseInt(b,10);a=createStorageObject();for(var c=0;c<a.length;c++)if(a[c].getStorage()===b){if(confirm("Are you sure you want to permanently remove the entry for "+a[c].getName()+"?")===true){localStorage.removeItem(a[c].getStorage());window.location.reload()}break}return true}function clearAllData(){confirm("Are you sure you want to permanently remove all contacts?")===true&&localStorage.clear();window.location.reload();return true};